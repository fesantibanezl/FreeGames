    ...
    if (!compra) {
      destino.innerHTML = `
        <section class="access-notice">
          ...
          <a class="primary-link" href="${rutas.carrito ?? '/carrito/'}">Ir al carrito</a>
        </section>
      `;
      return;
    }
    ...
    destino.innerHTML = `
      <section class="purchase-success" aria-labelledby="purchase-title">
        ...
        <div class="form-actions">
          <a class="button button-primary" href="${rutas.misCompras ?? '/mis-compras/'}">Ver mis compras</a>
          <a class="button button-secondary" href="${rutas.inicio ?? '/'}">Seguir explorando</a>
        </div>
      </section>
    `;
  }
  ...
  function renderizarHistorial() {
    ...
    if (sesion.usuario.rol !== 'cliente') {
      ...
      guardia.innerHTML = `
        ...
        <a class="primary-link" href="${rutas.perfil ?? '/perfil/'}">Ir a mi perfil</a>
      `;
      contenido.hidden = true;
      return;
    }
    ...
    if (compras.length === 0) {
      contenido.innerHTML = `
        <section class="empty-state">
          ...
          <a class="primary-link" href="${rutas.inicio ?? '/'}">Explorar juegos</a>
        </section>
      `;
      return;
    }
